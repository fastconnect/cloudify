...
public class OpenStackNetworkClient extends OpenStackBaseClient {
     ...
     *             Thrown when something went wrong with the request.
     */
    public String createAndAssociateFloatingIp(final String deviceId, final String networkId) throws OpenstackException {
        final String floatingNetworkId = this.getPublicNetworkId();
        return this.createAndAssociateFloatingIp(deviceId, networkId, floatingNetworkId);
    }

    public String createAndAssociateFloatingIp(final String deviceId, final String networkId, final String floatingNetworkId)
            throws OpenstackException {
        if (logger.isLoggable(Level.FINE)) {
    ...
    /**
     * Retrieve a network id which is connected to the external world given the public network name.
     * 
     * @return The public network id or <code>null</code> if not found.
     * @throws OpenstackException
     *             Thrown when something went wrong with the request.
     */
    public String getPublicNetworkIdByName(String networkName) throws OpenstackException {
        final List<Network> networks = this.getPublicNetwork();
        for (final Network network : networks) {

...
import org.cloudifysource.esc.driver.provisioning.storage.AzureRemoteStorageProvisioningDriverAdapter;
...
public class ElasticMachineProvisioningCloudifyAdapter implements ElasticMachineProvisioning, Bean {
	...
	private AzureStorageProvisioningDriver azureStorageProvisioning;
	...
	public void afterPropertiesSet() throws Exception {
		...
		try {
			...
			try {
				...
				//
				// logger.log(Level.INFO,String.format("Failed to read cloud directory path in '%s' retry with '%s'",
				...
				final String azureStorageClassName = this.cloud.getConfiguration().getAzureStorageClassName();
				if (StringUtils.isNotBlank(azureStorageClassName)) {
					...
					logger.info("creating Azure storage provisioning driver.");
					this.azureStorageProvisioning = (AzureStorageProvisioningDriver) builder
							.build(cloudConfigDirectoryPath, azureStorageClassName);
					logger.info("storage Azure provisioning driver created successfully.");
				}
	...
	public Object getExternalApi(final String apiName) throws InterruptedException,
			ElasticMachineProvisioningException {
		logger.finest("Requesting external api named = " + apiName);
		...
		} else if (apiName.equals(CloudifyConstants.AZURE_REMOTE_API_KEY)) {
			externalApi = new AzureRemoteStorageProvisioningDriverAdapter(azureStorageProvisioning);
		}
		return externalApi;
	}
	 ...
	 * @param serviceName
	 *            the service name
	 * @param previousMachineDetails
	 *            the MachineDetails of the failed machine
